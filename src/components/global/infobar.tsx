  ...
  const handleClick = () => {
    if (!showAll) {
      ...
      if (notifications?.length !== 0) {
        setAllNotifications(
            notifications?.filter((item: { subAccountId: string | null }) => item.subAccountId === subAccountId) ?? []
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <Sheet>
            ...
            <SheetContent className="mt-4 mr-4 pr-4 overflow-scroll">
              ...
              {allNotifications?.map((notification: { id: React.Key | null | undefined; User: { avatarUrl: any; name: string }; notification: string; createdAt: string | number | Date }) => (
                <div
                  key={notification.id}
